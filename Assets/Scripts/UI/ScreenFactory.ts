...
export class ScreenFactory {
  ...
  constructor(uiController: UIController, eventBus: EventBus, logger: Logger) {
    ...
    this.logger = logger
    this.eventBus.on<AppScreen>("screenChanged", (screen) => {
      if (!this.getStartedScreen) return
      if (screen === AppScreen.GetStarted) {
        this.getStartedScreen.show()
      } else {
        this.getStartedScreen.hide()
      }
    })
  }
  ...
  /** Convenience: update active Circuit button label on InArea screen. */
  setCircuitName(name: string): void {
    if (this.inAreaScreen) {
      this.inAreaScreen.setCircuitName(name)
    }
  }
  ...
  /** Convenience: lock/unlock InArea authoring while spatial area is resolving. */
  setAreaReady(ready: boolean): void {
    if (this.inAreaScreen) {
      this.inAreaScreen.setAreaReady(ready)
    }
  }

  /** Convenience: update route/layer summary text on InArea screen. */
  setRouteSummary(text: string): void {
    if (this.inAreaScreen) {
      this.inAreaScreen.setRouteSummary(text)
    }
  }
