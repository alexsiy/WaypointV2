...
export class WatchWidget extends WidgetBase {
  ...
  applyCompactLayout(): void {
    ...
    this.timeText.worldSpaceRect = Rect.create(-6, 6, -1.6, 1.6)
    this.timeText.horizontalOverflow = HorizontalOverflow.Shrink
    this.timeText.verticalOverflow = VerticalOverflow.Overflow
    this.timeText.horizontalAlignment = HorizontalAlignment.Center
    this.timeText.verticalAlignment = VerticalAlignment.Center
    this.timeText.renderOrder = 10
    this.timeText.getSceneObject().getTransform().setLocalPosition(
      new vec3(0, 0, 0.12)
    )
  }
}
